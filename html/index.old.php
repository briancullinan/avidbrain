<?php
	...
	include(APP_PATH.'functions/global.functions.php');
	...
	if(isset($app->dependents->DEBUG) && !empty($app->dependents->DEBUG) || DOMAIN=='http://avidbrain.dev'){
		$config['debug'] = true;
	...
	if(isset($minifyme)){
		...
		$versionFile = APP_PATH.'dependents/version.php';
		...
		$minifier->minify(DOCUMENT_ROOT.'css/final.'.$nextVersion.'.css');

		$minifier = new Minify\JS();
		foreach($app->header->localjs as $jsfiles){
			$minifier->add(DOCUMENT_ROOT.'js/'.$jsfiles);
		}
		$minifier->minify(DOCUMENT_ROOT.'js/final.'.$nextVersion.'.js');
		notify('ALL DONE');
		exit;
	}
	...
	include(APP_PATH.'validate/validate.post.php');
	include(APP_PATH.'routes/page.routes.php');
	$app->add(new \Zeuxisoo\Whoops\Provider\Slim\WhoopsMiddleware);
	include(APP_PATH.'debug/debug.php');
	include(APP_PATH.'debug/fixes.php');
	$app->run();

$(document).ready(function() {
	...
	//$('.picker__select--year').prepend('<option value="2013">2013</option>');
	...
	$('#upload-select').on('click',function(){
		...
		$('#upload-clicker').on('change',function(){
			...
			//$('#field_becomeatutor_myresume .anything').val('uploadfile');
	...
	$('.how-it-works').on('click',function(){
		...
		if(datastatus=='closed'){
			$('.how-it-works').attr('data-status','open');
	...
	$('#pagewidth').val(($('.the-tops').outerWidth()-40));
	...
	$('.button-form-switch button').on('click',function(){
		...
		//$(this).closest('form').submit().fadeOut();
	});

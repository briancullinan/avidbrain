...
<?php if(isset($app->history)): ?>
	...
	<?php foreach($app->history as $history): ?>
		...
		<tr>
            ...
            <td>
                <?php
                    if(isset($history->taxes)){
                        $taxremoval = (((stripe_transaction($history->amount))-$history->amount)/100);
                        $totalCost = $history->amount/100;
                        $final = ceil($totalCost - $taxremoval);
                        $finalPercent = (($final * $history->payrate)/100);
                    }
                    else{
                        $finalPercent = (($history->amount * $history->payrate)/10000);
                    }
                ?>

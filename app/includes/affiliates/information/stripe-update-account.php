<?php
    ...
    if(isset($app->affiliate->account_id) && empty($app->affiliate->managed_id)){


            $managed = \Stripe\Account::create(
    			array(
    				'from_recipient'=>$app->affiliate->account_id
    			)
    		);

    		$app->connect->update('avid___affiliates',array('managed_id'=>$managed->id),array('email'=>$app->affiliate->email));
    ...
    if(isset($app->affiliate->account_id)){
        try{
            $recipient = \Stripe\Recipient::retrieve($app->affiliate->account_id);
        }
        catch(Exception $e){
            //printer($e);
        }
...
<div class="block">
    ...
    <form method="post" class="form-post" id="verifyaccount" action="/affiliates/information">
